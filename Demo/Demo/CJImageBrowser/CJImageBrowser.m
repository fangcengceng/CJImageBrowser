...
@interface CJImageBrowser ()<CJScrollImageViewDelegate>
{
    // 滚动的view
...
- (void)createToolbar
{
    ...
    _toolbar = [[CJImageToolbar alloc] init];
...
#pragma mark - CJScrollImageView代理
...
- (void)showPhotos
{
    ...
    for (CJScrollImageView *photoView in _visiblePhotoViews) {
...
{
    CJScrollImageView *photoView = [self dequeueReusablePhotoView];
    if (!photoView) { // 添加新的图片view
        photoView = [[CJScrollImageView alloc] init];
    ...
    CJImageInfo *photo = _photos[index];
...
{
    if (index > 0) {
        CJImageInfo *photo = _photos[index - 1];
...
- (CJScrollImageView *)dequeueReusablePhotoView
{
    CJScrollImageView *photoView = [_reusablePhotoViews anyObject];
    if (photoView) {
